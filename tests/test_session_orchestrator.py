...
def _schedule_query_result() -> SimpleNamespace:
    return SimpleNamespace(
        lessons=(),
        changes=(),
        state=SimpleNamespace(),
...
def test_query_schedule_with_session_reuse_logs_in_when_no_saved_cookies(
    monkeypatch,
) -> None:
    previous_state = _runtime_state()
    config = _config()
    ...
    def fake_run_schedule_query(*args: object, **kwargs: object) -> SimpleNamespace:
        run_calls.append(kwargs)
        return _schedule_query_result()

    monkeypatch.setattr(app_module, "_login", fake_login)
    monkeypatch.setattr(app_module, "run_schedule_query", fake_run_schedule_query)

    result = app_module.query_schedule_with_session_reuse(
        config,
        force_email=False,
        trust_env=True,
    )
...
def test_query_schedule_with_session_reuse_reuses_saved_cookies_without_login(
    monkeypatch,
) -> None:
    previous_state = _runtime_state(
        session_cookies={"JSESSIONID": "saved"},
...
def test_query_schedule_with_session_reuse_retries_once_after_session_expiry(
    monkeypatch,
) -> None:
    previous_state = _runtime_state(
        session_cookies={"JSESSIONID": "saved"},
    ...
    def fake_run_schedule_query(*args: object, **kwargs: object) -> SimpleNamespace:
        run_calls.append(kwargs)
        if len(run_calls) == 1:
            raise QueryError("JWXT schedule list request failed with HTTP 901.")
        return _schedule_query_result()
...
def test_query_schedule_with_session_reuse_does_not_retry_non_session_error(
    monkeypatch,
) -> None:
    previous_state = _runtime_state(
        session_cookies={"JSESSIONID": "saved"},
    ...
    with pytest.raises(QueryError, match="retry attempts"):
        app_module.query_schedule_with_session_reuse(
            config,
            force_email=False,
            trust_env=True,
        )
