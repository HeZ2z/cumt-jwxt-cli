...
def test_output_config_resolve_dir_uses_explicit_directory() -> None:
    config = OutputConfig(save_json=False, save_report=False, output_dir="~/exports")

    assert config.resolve_dir(Path("/tmp/config.local.json")) == Path(
        "~/exports"
    ).expanduser()


def test_output_config_resolve_dir_uses_config_adjacent_default() -> None:
    config = OutputConfig(save_json=False, save_report=False, output_dir="")
